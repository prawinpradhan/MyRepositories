...
if(inputLength >= 3) {
 ...
 if(length(predictedDF)== 0 ){
   ...
   ggplotObj<- ggplot(predictedDF,aes(x=reorder(Predict,Score),y=Score,fill=Score)) + geom_bar(stat="identity") +xlab("Predicton") + coord_flip()
   return(list(predictedDF$Predict[1],ggplotObj))
}  else if(length(predictedDF$Predict) == 1 ){ 
  ggplotObj<- ggplot(predictedDF,aes(x=Predict,y=Score)) + geom_bar(stat="identity",fill="blue") +xlab("Predicton") + coord_flip()
  return(list(predictedDF$Predict[1],ggplotObj))
}
}
